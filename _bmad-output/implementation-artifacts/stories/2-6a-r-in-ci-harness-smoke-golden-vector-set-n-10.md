...
- [ ] **Task 1 — Author `tests/golden/regenerate.R`** (AC: 2)
  - [ ] Pin `set.seed(20260514)` at top of script.
  - [ ] Parse `--smoke` / `--full` CLI args via `commandArgs(trailingOnly = TRUE)`.
  - [ ] For `--smoke`: read `tests/golden/vectors-smoke.json` via `jsonlite::fromJSON`; iterate 6 entries.
  - [ ] For each entry: construct a minimal mirt 2PL model with the entry's items + invoke `fscores` with `method="EAP"`, `quadpts=61`, `theta_lim=c(-6, 6)`, `response.pattern=<matrix>`.
  - [ ] Collect `rTheta` (point estimate) + `rSE` (SE attribute) per entry; round to 6 decimals.
  - [ ] Write to `tests/golden/r-output-smoke.json` as a sorted JSON array.
  - [ ] For `--full`: print "not yet implemented; see Story 2.6b" + exit 0.

- [ ] **Task 2 — Author `tests/golden/parity-audit.mjs`** (AC: 3)
  - [ ] Load both fixture files via `readFileSync` + `JSON.parse`.
  - [ ] If `r-output-smoke.json` missing → `console.error(...)` + `process.exit(1)`.
  - [ ] For each entry: compute drift magnitudes; collect divergent entries.
  - [ ] Pass: `console.log('parity-audit: ok (...)')` + `process.exit(0)`.
  - [ ] Fail: per-entry drift report to stderr + `process.exit(1)`.

- [ ] **Task 3 — Author `.github/workflows/golden-regen.yml`** (AC: 1)
  - [ ] Triggers: `workflow_dispatch` + `pull_request: types: [labeled]` (with `regen-goldens` label condition).
  - [ ] Job: `setup-r@v2` → `setup-r-dependencies@v2` → `Rscript tests/golden/regenerate.R --smoke` → `node tests/golden/parity-audit.mjs`.
  - [ ] Cache key: `${{ runner.os }}-r-4.4-mirt-1.41`.
  - [ ] Job timeout: 15 min.

- [ ] **Task 4 — Author `tests/golden/README.md` + `CHANGELOG.md`** (AC: 4, 5)
  - [ ] README documents regen pipeline + audit history + version pins.
  - [ ] CHANGELOG documents v0.1 → v0.2 → v1.0 evolution.

- [ ] **Task 5 — Author `tests/unit/golden/parity-audit.test.mjs`** (AC: 7)
  - [ ] Three assertions per AC-7 sub-list.
  - [ ] Uses only `node:test` + `node:assert/strict` + temp-file fixtures.

- [ ] **Task 6 — Run full local pipeline** (AC: 9, 10)
  - [ ] `make lint` → 0.
  - [ ] `make test` → 0 (all existing tests + new parity-audit unit tests green).
  - [ ] `actionlint .github/workflows/golden-regen.yml` if available (skip if not installed locally; CI will catch).

- [ ] **Task 7 — First CI run + outcome documentation (DEFERRED, post-merge)** (AC: 6)
