...
### [blocker] `make test` is RED on epic/6 tip: tests/scaffold/ci-matrix.test.mjs AC-3 ("deferred jobs carry if:false") fails (suite 974 tests, 972 pass, 1 fail, 1 skip). Story 6.6 activated the `cropping-fuzzer` CI job (AC-7) and Story 6.4 activated the `lighthouse` job (AC-11) by removing their `if: false` stubs in .github/workflows/pr-checks.yml, but the class-A frozen scaffold test ci-matrix.test.mjs was only updated to add the 6.1/6.2 jobs (co-equal-triplet-computed-style, reveal-stage-event-ordering, difficulty-sentence) to EPIC_1_ACTIVE — lighthouse and cropping-fuzzer were never moved out of the "deferred" set, so AC-3 still asserts they carry if:false and goes red. This is a self-inflicted epic-6 regression (both jobs are if:false on main, test green there), not a pre-existing failure as the 6-4/6-5/6-7 self-reviews claimed. A red node-test suite blocks tds deliver / CI on the very job-matrix contract guard.
