...
- [ ] **Task 1: author the scaffold guard** (`tests/scaffold/9c-1-ru-translator-outreach.test.mjs`) encoding AC 5; confirm RED. (test-author phase)
- [ ] **Task 2: write the recruitment outreach draft** (AC 1). (impl phase)
- [ ] **Task 3: write the outreach log** (AC 2) and add a back-link from `ru-translator-signoff.md` if appropriate (AC 3). (impl phase)
- [ ] **Task 4 (BLOCKED — human deliverable): engage the translator, author native RU copy, obtain sign-off** (AC 4). No agent action; copy must be native-authored, not AI-translated.
