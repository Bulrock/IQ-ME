...
| 4 | tds deliver (**auto-deliver default ON**) | **Default behavior — fires immediately после apply-verdict approved/approved-with-deferred.** На этой точке stories уже at `approved` (flipped в Step 3 verdict); epic тоже at `approved` (apply-verdict flips epic `in-progress → approved` параллельно). Печатает план одной строкой (epic_branch + commit count + delivery flips list) и **immediately** invokes `tds deliver --as=engineer --epic=<id> --epic-branch=<branch> --title=<text> --body=<text>` без halt-confirm. <br/>**Opt-out:** pass `--no-auto-deliver` к code-review invocation (`/bmad-tds-code-review --epic=<id> --no-auto-deliver`) — restores legacy halt-confirm. Что показывает halt: epic_branch + status_key, commit count (`git rev-list --count main..epic_branch`), список stories (`approved → done`) + epic (`approved → done`), необратимо: push remote → PR open (or reuse existing) → squash-merge → 1 коммит в main → engineer flips всех sprint-status keys + atomic finalization commit на default branch → удаление epic_branch локально. На «yes» → fires deliver. На «no» → halt (status остаётся `approved`, findings уже recorded в Step 2). <br/>**Tds deliver behavior** (одинаково для default + opt-out): pre-merge validation rejects если хоть одна story / epic не at `approved` (или `done` для idempotent re-run); auto-merge default true; idempotent на existing PR; sprint-status flips through engineer role; на GitLab CI-async case есть встроенный poll-loop default 30 минут — `--poll-timeout=1h` для длинных pipeline'ов; transient HTTP errors 405/502/503/409 retry'ятся 3× с 5/10s backoff. **Post-merge state finalization (OH-07):** после успешного squash merge `tds deliver` сам switch'ит на default branch (`git checkout main && git fetch && git pull --rebase`), пишет post-merge sprint-status flips (stories+epic → `done`), commit'ит их **на default branch** через `autoCommitTdsState`, push'ит к origin с pull/rebase retry на non-fast-forward (concurrent contributors). Это closes pre-OH-07 bug pathway где flips landed на epic_branch и терялись при cleanup. **Branch-protection fallback:** если direct push к default branch denied (GH006, required reviews, etc.) — default mode возвращает non-zero exit с structured envelope (`finalizationOnDefaultBranch.pending.reason="push-denied-by-protection"`); opt-in `--finalization-pr=auto` создаёт short-lived branch `chore/state-finalization/<epic-id>` + opens PR против default carrying the same finalization commit. **Никогда** silent dirty-state на epic_branch. **Partial-failure visibility:** если deliver упал после успешного squash merge но не смог flip какие-то story / epic статусы — exit non-zero + structured stderr envelope с recovery hint; sprint-status оставлен в смешанном `approved` / `done` состоянии, re-run safely завершает flips. **Idempotent re-run:** transient HTTP error / manual UI merge через GitLab/GitHub web / любой partial halt — просто **re-run** code-review с теми же args; `tds deliver` detect'ит already-merged PR через `getStatus`, эмитит `kind=deliver-resumed` в pr-events, skip'ает push/squash phases и завершает finalization + flips + cleanup. <br/>**Rationale auto-deliver default-ON:** typical solo-dev flow всегда answers «yes» — halt-confirm чисто friction. Memory `feedback_review_binary_no_autofix.md` design (binary verdict explicit approve) preserved через apply-verdict (Step 3) — verdict ALREADY explicit; Step 4 — mechanical follow-through. Opt-out flag для team / shared environments / contractor handoffs где human gate value > friction. |
| 5 | emit telemetry | state-transitions для всех stories, branch-registry updates, review-events JSONL. |
| 6 | **state-commit sweep** | Финальный workflow shag — `tds state-commit -m "chore(<epic-id>): code-review Mode 2 sweep + deliver" --story=<epic-id> --as=engineer`. **OH-07 post-2026-05-14 scope:** sweep больше **не** captures `tds deliver` post-merge sprint-status flips — `tds deliver` Step 4 уже committed их **inline на default branch** (см. Step 4 «Post-merge state finalization»). Sweep теперь captures только non-deliver mutations: Phase 1 auto-commits residue в add-finding handlers, story-md drift outside add-finding cycle, branch-registry transitions on non-deliver code paths. Idempotent + never-throws — typically no-op после auto-delivery success. | — (CLI) |
...
- **`tds deliver` — единственный путь к squash merge `epic_branch → main`** (Mode 2). Manual `gh pr merge` через UI ALSO supported: re-run `tds deliver` с теми же args — host PR/MR в state=merged detected через `findPRForBranch({ states: ["open", "merged"] })`, и deliver резюмирует finalization (status flips + branch cleanup) без повторного push/PR/squash.
