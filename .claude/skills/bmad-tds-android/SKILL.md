...
4. **Execute** — TDD cycle:
   - Red: JUnit + AssertJ (recommended) или JUnit + truth/AssertJ; Compose UI tests если applicable. `./gradlew :module:test` → red. Test invocation gotchas:
     - **Module-specific path обязателен.** `./gradlew test` на multi-module project запускает test task root project'а который часто **не находит actual tests** (особенно с AGP 8+ structure). Используй `./gradlew :app:test` или `./gradlew :feature-login:testDebugUnitTest` — explicit module + variant.
     - **Gradle 9 «no tests discovered» trap.** Gradle 9.0 removed deprecated convention где Test task inherited `testClassesDirs` + `classpath` из 'test' source set. На un-migrated projects `:module:test` фейлится «no tests discovered» даже если тесты есть. Source: https://docs.gradle.org/current/userguide/upgrading_major_version_9.html — «`Test` tasks may no longer execute expected tests». Recovery: explicit `testClassesDirs` + `classpath` в build.gradle.kts или test-suites DSL.
     - **Unit (JVM, fast) vs Instrumented (device, slow):** `testDebugUnitTest` — JVM на хосте, no Android framework (Robolectric optional). `connectedDebugAndroidTest` — нужен running emulator / device, full Android runtime. НЕ смешивай: unit tests в `src/test/`, instrumented в `src/androidTest/`. Agent stumble: класть Android-framework-dependent test в `src/test/` → NullPointerException на каждом `Context.getString(...)`.
     - **JUnit 5 на Android instrumented (`src/androidTest/`)** требует de Mannodermaus's `android-junit5` Gradle plugin (https://github.com/mannodermaus/android-junit5) — without it AndroidJUnitRunner default'ится на JUnit 4 и silent skip'ит JUnit 5 tests. Unit-side (`src/test/`) JUnit 5 работает out-of-the-box с `useJUnitPlatform()` в test task config.
     - **Configuration cache (Gradle 8.1+ stable, 9+ default).** Если build.gradle.kts использует deprecated APIs — gradle warns и cache misses каждый раз. Agent stumble: видит «Configuration cache state could not be cached» и игнорирует — теряем 10×+ speedup. Address: либо fix deprecated calls, либо `--no-configuration-cache` для unblock с TODO.
