...
description: |
  Epic dev-phase orchestrator (Full mode only). Активируется на phrases: «execute epic E-2», «start epic», «work on epic X». Создаёт epic-integration branch, итеративно вызывает execute-story per story → каждая sequentially squash-merge'ит свой story_branch в epic_branch (внутри execute-story Step 6) и остаётся в status=review. Epic_branch → main delivery решает clean-context auditor через /bmad-tds-code-review Mode 2 + tds deliver.
---

# bmad-tds-execute-epic

Epic dev-phase orchestrator. Создаёт epic_branch, итеративно прогоняет каждую story через `/bmad-tds-execute-story` до `status=review`. **Каждая story sequentially squash-merge'ится в epic_branch внутри `execute-story` Step 6** — epic_branch постепенно растёт (N squash-коммитов = N stories), cross-story dependencies резолвятся natural'но (следующая story dev'ится от обновлённого epic_branch tip). Epic_branch → main delivery остаётся за clean-context auditor (`/bmad-tds-code-review` Mode 2 + `tds deliver`).
...
| 6 | **state-commit sweep** | После Step 5 — `tds state-commit -m "chore(<epic-id>): execute-epic session sweep" --story=<epic-id> --as=engineer`. Sweep аккумулирует epic-level state mutations что не покрыли child execute-story sweeps (например epic-branch registry attach/start, sprint-status epic flip). Idempotent — typically no-op если каждый child execute-story session корректно sweep'нул свои mutations. **Cross-ref OH-07:** post-merge `approved → done` finalization для stories+epic выполняется **не здесь** (этот workflow только до status=review) — finalization landed в `tds deliver` Step 4 code-review Mode 2 atomic'но на default branch (см. `bmad-tds-code-review` SKILL.md). | — (CLI) |
